...
struct User {
    id: Option<i32>, // id is an integer and is optional. The reason is that we don't provide the id when we create or update a new user. The database will generate it for us. But we still want to return the user with an id when we get them.
    name: String,
    email: String,
}
...
const TEST: &str = env!("TEST"); // docker-compose DATABASE_URL_ARG => Dockerfile DATABASE_URL
...
fn main(){
    ...
    let listener = TcpListener::bind(format!("0.0.0.0:8080")).unwrap(); // TcpListener::bind is used to create a TCP server on port 8080.
    ...
    for stream in listener.incoming(){
        match stream {
            Ok(stream) => {
                handle_client(stream); // whenever an incoming connection comes in, it's redirected to handle_client function
            }
            Err(e) => {
...
fn set_database() -> Result<(), PostgresError> {
    println!("Env var DB_URL is: {}", DB_URL);
    println!("Env var TEST is: {}", TEST);
    ...
    match stream.read(&mut buffer) {
        Ok(size) => {
            ...
            let (status_line, content) = match &*request {
                r if r.starts_with("POST /users") => handle_post_request(r), // CREATE
                r if r.starts_with("GET /users/") => handle_get_request(r), // READ
                r if r.starts_with("GET /users") => handle_get_all_request(r), // READ ALL
                r if r.starts_with("PUT /users/") => handle_put_request(r), // UPDATE
                r if r.starts_with("DELETE /users/") => handle_delete_request(r), // DELETE
                _ => (NOT_FOUND.to_string(), "404 not found".to_string()), // If we don't have a match, we send back a 404 error.
            };
